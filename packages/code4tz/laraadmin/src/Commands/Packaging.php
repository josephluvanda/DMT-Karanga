...
class Packaging extends Command
{
    ...
    var $modelsInstalled = ["User", "Role", "Permission", "Employee", "Department",
    "Upload", "Organization", "Backup", "Registration", "District", "Region", "Ward"];
    ...
    public function handle()
    {
        ...
        $from = base_path();
        $to = base_path('packages/code4tz/laraadmin/src/Installs');
        ...
        $appModels = config('dmt.models');
        // Merge DTM Models
        if( $appModels != null OR !empty($appModels)){
          $this->modelsInstalled = array_merge($this->modelsInstalled, $appModels);
        }
        ...
        $this->copyFile($from."/config/dmt.php", $to."/config/dmt.php");
        ...
        $this->replaceFolder($from."/database/seeds", $to."/seeds");
